...
{
    char question[81], answer[81], input_question[81];
    FILE *fp, *fp_history;
    ...
    // continue prompting until they type exit
    do
    {
        printf(ANSI_BLUE "Here are questions you can ask:\n" ANSI_OFF);
        printf(ANSI_BLUE "Please type the question in full. (capitalization not necessary)\n\n" ANSI_OFF);

        printf("1. Is vaccination necessary?\n"
               "2. What are the possible side effects of vaccination?\n"
               "3. Am I still fully vaccinated if I dont have a third dose?\n"
               "4. Do boosters use the same ingredients as the vaccine?\n"
               "5. How do vaccines work?\n"
               "6. How do vaccines differ?\n"
               "7. Why do we need to get vaccinated for COVID-19?\n"
               "8. Why will the prioritized groups get the vaccine first?\n"
               "9. Is the vaccine free for priority groups?\n"
               "10. Are there risks for complications when given the COVID-19 vaccine?\n\n");

        fflush(stdin);
        printf(ANSI_BLUE "Ask me a question (type 'exit' to quit): " ANSI_OFF);
        ...
        int line_number = 0;
        while (fgets(question, 81, fp))
        {
            memset(input_question, 0, sizeof(input_question)); // reset input
            fgets(answer, 81, fp);
            ...
            if (strstr(lower_question, lower_q) != NULL)
            {
                if (line_number % 2 == 0)
                {
                    printf("Accessed Question: %s\n", question);
                }

                printf(ANSI_RED "[AssistiVax]: %s\n" ANSI_OFF, answer);
        ...
        if (!found)
        {
            printf(ANSI_YELLOW "Sorry, I don't know the answer. Please type another question.\n" ANSI_OFF);
            fprintf(fp_history, "%s\n", input_question);                                              // write question to file
    ...
    } while (strcmp(input_question, "exit") != 0);
