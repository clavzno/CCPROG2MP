...
struct user {
  ...
  Str10 dose1_loc;
  // Optional
  /* note: prompt user if they have more than 1 dose, if 0, set all to N/A or null*/
  Str10 dose2_date;
  Str10 dose2_type;
  Str10 dose2_loc;
  Str10 dose3_date;
...
void checkEmpty(
    struct *userProfilesptr,
    struct *userAmountptr); // checks for empty profile and returns iteration
                            // for direct manipulation of that index
void checkID(int id, struct *userAmountptr); // checks for any repeated IDs
void checkPassword(
    char password[],
    struct *userAmountptr); // makes the user confirm their entered password
