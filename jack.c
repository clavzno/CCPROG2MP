...
Notes: (1) won't run in Repl.it console, (2) vsCode Terminal won't show flashing
colors, (3) Win11 Terminal shows no difference between bold and non-bold colors.
...
int vaxreg_userreg_checkID(struct user *Profiles, int *num_usersptr,
                           int tempid) {
  int i;
...
int vaxreg_useregistration(struct user *Profiles, int *num_usersptr) {
  ...
  int emptyprofile =
      0;           // stores the return value of vaxreg_userreg_checkifempty
  ...
  if (*num_usersptr < MAX_USERS) {
    ...
    if (addnewuser == 1) {
      ...
      if (emptyprofile < 0) /* NOTE REMOVE THIS BECAUSE IT WOULDNT MOVE FORWARD
                               ANYWAY F IT WAS FULL */
      {
        ...
        while (idcheck > 0) {
          printf(ANSI_RED
                 "Error: userID already taken in Profile %d.\n" ANSI_OFF,
                 idcheck);
          sleep(2);
          printf("\r");
          tempid = 0;
        ...
        do {
          ...
          passcheck =
              vaxreg_userreg_checkpasswordlen(Profiles[emptyprofile].password);
        ...
        do {
          ...
          fgets(Profiles[emptyprofile].address, 31,
                stdin); // fgets is used to read strings with spaces
        ...
        } else { // set dose2and dose3 fields to "N/A"
...
user and appointment files are saved, preserved, and can be edited. Every
chatbot interaction including questions and answers are saved and preserved. */
...
void acc_bgcolor() {
} // allows user to choose between darkmode and lightmode for the terminal
...
int menu_main(struct user *Profiles, int *num_usersptr,
              struct settings *setptr) {
  int choice;
  printf("**************************\n");
  printf("\n");
  printf(ANSI_WHITE "Welcome to" ANSI_OFF ANSI_FLASH_RED
                    " Vax" ANSI_OFF ANSI_FLASH_BLUE
                    "Bot" ANSI_OFF ANSI_FLASH_WHITE "PH!" ANSI_OFF "\n");
  printf("\n");
  printf("Select Menu:\n");
